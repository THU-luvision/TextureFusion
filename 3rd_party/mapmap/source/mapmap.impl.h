...
throw(std::runtime_error)
{
    ...
    if (!hint) std::fill(m_solution.begin(), m_solution.end(), 0);
    else m_solution.assign(solution.begin(), solution.end());

    /* find initial solution by tree-optimization w/o dependencies */
    m_objective = initial_labelling(hint);
...
reset()
{
    m_objective = static_cast<_s_t<COSTTYPE, SIMDWIDTH>>(0);
    m_hist_energy.clear();
    m_hist_time.clear();
    m_hist_mode.clear();
    m_hist_acyclic_iterations = 0;
