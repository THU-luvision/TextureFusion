...
public:
    ...
    _s_t<COSTTYPE, SIMDWIDTH> optimize(std::vector<_iv_st<COSTTYPE, SIMDWIDTH>>&
        solution) throw(std::runtime_error);

    /* start optimization with customized control flow */
    _s_t<COSTTYPE, SIMDWIDTH> optimize(std::vector<_iv_st<COSTTYPE, SIMDWIDTH>>&
        solution, const mapMAP_control& control_flow, bool hint = false) throw(std::runtime_error);
...
protected:
    ...
    _s_t<COSTTYPE, SIMDWIDTH> initial_labelling(bool hint = false);
