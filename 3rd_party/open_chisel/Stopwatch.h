...
#define STOPWATCH(name, expression)                                         \
  do {                                                                      \
    const unsigned long long int startTime =                                \
...
#define TICK(name)                                              \
  do {                                                          \
    Stopwatch::getInstance().tick(                              \
        name, Stopwatch::getInstance().getCurrentSystemTime()); \
  } while (false)
...
class Stopwatch {
 public:
  ...
  void printAll() {
    for (std::map<std::string, float>::const_iterator it = timings.begin();
         it != timings.end(); it++) {
  ...
  void sendAll() {
    ...
    if ((currentSend = (clock.tv_sec * 1000000 + clock.tv_usec)) - lastSend >
        SEND_INTERVAL_MS) {
      int size = 0;
      stopwatchPacketType *data = serialiseTimings(size);
      sendto(sockfd, data, size, 0, (struct sockaddr *)&servaddr,
             sizeof(servaddr));
  ...
  static unsigned long long int getCurrentSystemTime() {
    ...
    unsigned long long int time =
        (unsigned long long int)(tv.tv_sec * 1000000 + tv.tv_usec);
    return time;
  }
 ...
 private:
  ...
  stopwatchPacketType *serialiseTimings(int &packetSize) {
    ...
    *((unsigned long long int *)&dataPacket[1]) = signature;
    ...
    float *valuePointer =
        (float *)&((unsigned long long int *)&dataPacket[1])[1];

    for (std::map<std::string, float>::const_iterator it = timings.begin();
         it != timings.end(); it++) {
      valuePointer = (float *)mempcpy(valuePointer, it->first.c_str(),
                                      it->first.length() + 1);
