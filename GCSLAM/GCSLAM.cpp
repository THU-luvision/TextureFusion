...
void GCSLAM::select_closure_candidates(Frame &f,
                                       std::vector<int> &candidate_frame_index)

{
  ...
  std::sort(candidates.begin(), candidates.end(),
            greater<MILD::LCDCandidate>());
  for (int k = 0; k < fmin(candidates.size(), maxCandidateNum); k++) {
    //    cout << kflist[candidates[k].index].keyFrameIndex << " " <<
    //    candidates[k].salient_score << endl;
  ...
  for (int k = 0; k < candidate_frame_index.size(); k++) {
    candidates_str +=
        std::to_string(kflist[candidate_frame_index[k]].keyFrameIndex) + " ";
  }
  //  cout << "running frame : " << f.frame_index << " " << candidates_str <<
  //  endl;
}

void GCSLAM::update_keyframe(
    int newKeyFrameIndex,
    MultiViewGeometry::FrameCorrespondence &key_frame_corr,
    float average_disparity, PoseSE3d relative_pose_from_key_to_new,
  ...
  std::vector<MultiViewGeometry::FrameCorrespondence> &fCorrList_keyframes =
      keyFrameCorrList;
  std::vector<MultiViewGeometry::KeyFrameDatabase> &kflist = KeyframeDataList;
  MILD::LoopClosureDetector &lcd = mild;
  std::vector<Frame> &frame_list = globalFrameList;
  ...
  for (size_t k = 0; k < candidate_keyframes.size(); k++) {
    ...
    MultiViewGeometry::FrameCorrespondence global_frame_corr(
        frame_list[candidate_frame_index], frame_list[newKeyFrameIndex]);
    fCorrCandidate.push_back(global_frame_corr);
    //    std::cout << "candidate key frame: " <<
    //    kflist[candidate_keyframes[k]].keyFrameIndex << std::endl;
  }
  ...
  for (size_t k = 0; k < candidate_keyframes.size(); k++) {
    ...
    registration_success_list[k] = MultiViewGeometry::FrameMatchingTwoViewRGB(
        fCorrCandidate[k], camera_para, sparseMatcher,
        relative_pose_from_ref_to_new_list[k], average_disparity_list[k],
        scale_change_ratio, update_keyframe_from_dense_matching);
    relative_pose_from_ref_to_new_list[k] =
        relative_pose_from_ref_to_new_list[k].inverse();
  }
  ...
  for (size_t k = 0; k < registration_success_list.size(); k++) {
    if (registration_success_list[k]) {
      kflist.back().corresponding_keyframes.push_back(
          fCorrCandidate[k].frame_ref.frame_index);
  ...
  for (size_t k = 0; k < fCorrCandidate.size(); k++) {
    ...
    if (min_average_disparity > average_disparity_list[k] &&
        registration_success_list[k]) {
  ...
  if (global_tracking_success == 1) {
    ...
    frame_list[newKeyFrameIndex].pose_sophus[0] =
        frame_list[fCorrCandidate[min_index].frame_ref.frame_index]
            .pose_sophus[0] *
  ...
  if (!global_tracking_success) {
    ...
    current_map_origin =
        *max_element(matched_frames.begin(), matched_frames.end());
  }
...
void GCSLAM::updateMapOrigin(
  ...
  for (size_t k = 0; k < fCorrCandidate.size(); k++) {
    if (registration_success_list[k]) {
      ...
      frame_list[newKeyFrameIndex].origin_index =
          std::min(ref_origin, current_origin);

      if (0) {
        if (ref_origin < current_origin) {
          for (int keyframeCnt = 0; keyframeCnt < kflist.size();
               keyframeCnt++) {
            if (frame_list[kflist[keyframeCnt].keyFrameIndex].origin_index ==
                current_origin) {
              frame_list[kflist[keyframeCnt].keyFrameIndex].origin_index =
                  ref_origin;
              frame_list[kflist[keyframeCnt].keyFrameIndex].is_keyframe = 0;
              for (int localframeCnt = 0;
                   localframeCnt <
                   kflist[keyframeCnt].corresponding_frames.size();
                   localframeCnt++) {
                frame_list[kflist[keyframeCnt]
                               .corresponding_frames[localframeCnt]]
                    .origin_index = ref_origin;
              }
            }
          }
        }
        if (current_origin < ref_origin) {
          for (int keyframeCnt = 0; keyframeCnt < kflist.size() - 1;
               keyframeCnt++) {
            if (frame_list[kflist[keyframeCnt].keyFrameIndex].origin_index ==
                ref_origin) {
              frame_list[kflist[keyframeCnt].keyFrameIndex].origin_index =
...
void GCSLAM::update_frame(Frame &frame_input) {
  ...
  //*********************** SparseMatcher is used for efficient binary feature
  // matching
  MILD::SparseMatcher sparseMatcher(FEATURE_TYPE_ORB, 32, 0, 50);
  ...
  if (anchor_frame >= 0 && frame_list[anchor_frame].tracking_success) {
    relative_transform_from_key_to_new =
        frame_list[anchor_frame].pose_sophus[0].inverse() *
        frame_list[last_keyframe_index].pose_sophus[0];
  }

  MultiViewGeometry::FrameCorrespondence key_frame_corr(
      frame_list[last_keyframe_index], f);
  bool update_keyframe_from_dense_matching = 0;

  //*********************** Match two frames based on RGBD features
  registration_success = MultiViewGeometry::FrameMatchingTwoViewRGB(
      key_frame_corr, camera_para, sparseMatcher,
      relative_transform_from_key_to_new, average_disparity, scale_change_ratio,
      update_keyframe_from_dense_matching, 1);
  int update_keyframe_flag = 0;

  if (((average_disparity > minimumDisparity && f.blur_flag == 0) ||
       (scale_change_ratio > 0.4)) &&
  ...
  if (registration_success && !update_keyframe_flag) {
    ...
    f.tracking_success = 1;
    f.pose_sophus[0] = frame_list[last_keyframe_index].pose_sophus[0] *
    ...
    kflist.back().relative_pose_from_key_to_current.push_back(
        relative_pose_from_key_to_new);
  }
  ...
  if (update_keyframe_flag) {
    local_tracking_cnt = 0;
    update_keyframe(f.frame_index, key_frame_corr, average_disparity,
                    relative_pose_from_key_to_new, registration_success);
    f.is_keyframe = 1;
    //*********************** fastBA for globally consistent pose estimation
    // TICK("GCSLAM::FastBA");
    MultiViewGeometry::optimizeKeyFrameMap(fCorrList_keyframes, frame_list,
                                           kflist, 0);
    // TOCK("GCSLAM::FastBA");
  }
}
