...
void optimize_3d_to_3d_huber_filter(
    Frame &frame_ref, Frame &frame_new,
    std::vector<cv::DMatch> &ransac_inlier_matches,
    PoseSE3d &relative_pose_from_ref_to_new,
    std::vector<float> &weight_per_point, float outlier_threshold = 0.015,
    int max_iter_num = 4, float huber_threshold = 0.008) {
  ...
  for (size_t i = 0; i < valid_3d_cnt; i++) {
    Eigen::Vector3d pt_ref(
        frame_ref.local_points[ransac_inlier_matches[i].queryIdx]);
    Eigen::Vector3d pt_new(
        frame_new.local_points[ransac_inlier_matches[i].trainIdx]);
    p_ref.push_back(pt_ref);
    p_new.push_back(pt_new);

    ref_points_vectorized[i] =
        _mm_setr_ps(pt_ref.x(), pt_ref.y(), pt_ref.z(), 1);
    new_points_vectorized[i] =
        _mm_setr_ps(pt_new.x(), pt_new.y(), pt_new.z(), 0);
    depth[i] = pt_ref.z();
  }
  //		float init_error = reprojection_error_3Dto3D(p_ref, p_new,
  // relative_pose_from_ref_to_new, 1);
  for (int iter_cnt = 0; iter_cnt < max_iter_num; iter_cnt++) {
    ...
    for (int i = 0; i < valid_3d_cnt; i++) {
      ...
      res =
          _mm_add_ps(res, _mm_dp_ps(T_ref[2], ref_points_vectorized[i], 0xf4));
      reprojection_error_vec = _mm_sub_ps(res, new_points_vectorized[i]);

      float error_sse =
          sqrt(_mm_cvtss_f32(_mm_dp_ps(res, res, 0x71))) / depth[i];
    ...
    if (delta_double.array().isNaN().maxCoeff() > 0 ||
        isnan(sin(delta_double.array().abs().maxCoeff()))) {
      cout << "nan detected in relative pose! " << endl;
      continue;
    }
    relative_pose_from_ref_to_new = Sophus::SE3d::exp(delta_double).inverse() *
  ...
  for (int i = 0; i < p_ref.size(); i++) {
    Eigen::Vector3d reprojection_error =
        applyPose(relative_pose_from_ref_to_new, p_ref[i]) - (p_new[i]);
...
void estimateRigid3DTransformation(Frame &frame_ref, Frame &frame_new,
  ...
  int count;
  int best_results = 0;
  float reprojection_error_threshold_square =
  ...
  for (size_t i = 0; i < N_total; i++) {
    Eigen::Vector3d ref_point =
        frame_ref.local_points[matches_before_filtering[i].queryIdx];
    Eigen::Vector3d new_point =
        frame_new.local_points[matches_before_filtering[i].trainIdx];
    ref_points_vectorized[i] = _mm_setr_ps(
        (float)ref_point[0], (float)ref_point[1], (float)ref_point[2], 1);
    new_points_vectorized[i] = _mm_setr_ps(
        (float)new_point[0], (float)new_point[1], (float)new_point[2], 1);
  ...
  for (int cnt = 0; cnt < max_iter_num; cnt++) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      ref_points[i] =
          frame_ref.local_points[init_matches[candidate_seed].queryIdx];
      new_points[i] =
          frame_new.local_points[init_matches[candidate_seed].trainIdx];
    ...
    mean_new_points /= 4;
    for (int i = 0; i < 4; i++) {
      H += (ref_points[i] - mean_ref_points) *
           (new_points[i] - mean_new_points).transpose();
    }
    ...
    for (int i = 0; i < 3; i++) {
      transform_vectorized[i] =
          _mm_setr_ps((float)temp_R(i, 0), (float)temp_R(i, 1),
                      (float)temp_R(i, 2), (float)temp_t(i));
    }
    count = 0;
    for (size_t i = 0; i < N_total; i++) {
      __m128 res;
      res = _mm_add_ps(
          _mm_dp_ps(transform_vectorized[1], ref_points_vectorized[i], 0xf2),
          _mm_dp_ps(transform_vectorized[0], ref_points_vectorized[i], 0xf1));
      res = _mm_add_ps(res, _mm_dp_ps(transform_vectorized[2],
                                      ref_points_vectorized[i], 0xf4));
      res = _mm_sub_ps(res, new_points_vectorized[i]);
      count += (_mm_cvtss_f32(_mm_dp_ps(res, res, 0x71)) / depth_square[i] <
                reprojection_error_threshold_square);
    }
...
float ransac3D3D(Frame &frame_ref, Frame &frame_new,
                 std::vector<DMatch> &init_matches,
                 std::vector<DMatch> &matches_before_filtering,
                 float reprojectionThreshold, int max_iter_num,
                 FrameCorrespondence &fCorr,
  ...
  estimateRigid3DTransformation(frame_ref, frame_new, init_matches, direct_R,
                                direct_t, reprojectionThreshold * 2,
                                max_iter_num);
  end = clock();
  ...
  for (size_t i = 0; i < matches_before_filtering.size(); i++) {
    ...
    Eigen::Vector3d normalized_predict_points =
        estimate_new_local_points / estimate_new_local_points(2);
    cv::KeyPoint predict_new_2D =
        frame_ref.keypoints[matches_before_filtering[i].queryIdx];
    cv::KeyPoint new_2D =
        frame_new.keypoints[matches_before_filtering[i].trainIdx];
    ...
    float average_reprojection_error_2d =
        sqrt(delta_x * delta_x + delta_y * delta_y);
    float reprojection_error =
        (new_point - direct_R * ref_point - direct_t).norm() / ref_point(2);
    ...
    if (reprojection_error < reprojectionThreshold * 2) {
      if (g_para.runTestFlag) {
        cout << i << " " << average_reprojection_error_2d << " "
             << reprojection_error << endl;
      }
  ...
  optimize_3d_to_3d_huber_filter(frame_ref, frame_new, ransac_inlier_matches,
                                 relative_pose_from_ref_to_new,
                                 weight_per_feature, reprojectionThreshold, 6,
                                 0.005);
  end = clock();
  ...
  Eigen::MatrixXd invH =
      relative_pose_from_ref_to_new.matrix().inverse().block<3, 4>(0, 0);
  ...
  for (size_t i = 0; i < matches_before_filtering.size(); i++) {
    ...
    predict_new_2D.pt.x = estimate_new_local_points(0) /
                              estimate_new_local_points(2) * para.c_fx +
                          para.c_cx;
    predict_new_2D.pt.y = estimate_new_local_points(1) /
                              estimate_new_local_points(2) * para.c_fy +
                          para.c_cy;
    predict_ref_2D.pt.x = estimate_ref_local_points(0) /
                              estimate_ref_local_points(2) * para.c_fx +
                          para.c_cx;
    predict_ref_2D.pt.y = estimate_ref_local_points(1) /
                              estimate_ref_local_points(2) * para.c_fy +
                          para.c_cy;
    float delta_x = predict_new_2D.pt.x - new_2D.pt.x;
    float delta_y = predict_new_2D.pt.y - new_2D.pt.y;
    float average_reprojection_error_2d_ref =
        sqrt(delta_x * delta_x + delta_y * delta_y);
    delta_x = predict_ref_2D.pt.x - ref_2D.pt.x;
    delta_y = predict_ref_2D.pt.y - ref_2D.pt.y;
    float average_reprojection_error_2d_new =
        sqrt(delta_x * delta_x + delta_y * delta_y);
    float reprojection_error =
        (new_point - estimate_new_local_points).norm() / ref_point(2);
    double reprojection_error_2d =
        average_reprojection_error_2d_ref;  // max(average_reprojection_error_2d_ref,
                                            // average_reprojection_error_2d_new);

    if (reprojection_error < reprojectionThreshold &&
        reprojection_error_2d < g_para.reprojection_error_2d_threshold) {
      average_reprojection_error_2D += reprojection_error_2d;
      if (g_para.runTestFlag) {
        cout << i << " " << average_reprojection_error_2d_ref << " "
             << average_reprojection_error_2d_new << " " << reprojection_error
             << endl;
      }
  ...
  if (g_para.runTestFlag) {
    cout << "2D/3D inlier num: " << ransac_2d_inlier_matches.size() << " "
         << init_matches.size() << endl;
  ...
  //      float reprojection_error = reprojection_error_3Dto3D(frame_ref,
  //      frame_new, ransac_inlier_matches, (relative_pose_from_ref_to_new), 0);
  ...
  //		outlierFiltering(frame_ref, frame_new, init_matches, 5,0.01);
  //    cout << "reprojection error after lp optimzation: " <<
  //    reprojection_error << endl;
  ...
  if (init_matches.size() > minimum_3d_correspondence) {
    ...
    fCorr.preIntegrate();
    float reprojection_error =
        reprojection_error_3Dto3D(fCorr, (relative_pose_from_ref_to_new));
    return reprojection_error;
  }
  return 1e6;
}

void outlierFiltering(Frame &frame_ref, Frame &frame_new,
  ...
  for (size_t i = 0; i < N; i++) {
    Eigen::Vector3d ref_point =
        frame_ref.local_points[init_matches[i].queryIdx];
    Eigen::Vector3d new_point =
        frame_new.local_points[init_matches[i].trainIdx];

    int distance_preserve_flag = 0;
    for (size_t j = 0; j < candidate_num; j++) {
      int rand_choice = rand() % N;
      Eigen::Vector3d ref_point_p =
          frame_ref.local_points[init_matches[rand_choice].queryIdx];
      Eigen::Vector3d new_point_p =
          frame_new.local_points[init_matches[rand_choice].trainIdx];
...
bool FrameMatchingTwoViewRGB(FrameCorrespondence &fCorr,
  ...
  float time_feature_matching, time_ransac, time_filter, time_refine,
  ...
  PoseSE3d init_guess_relative_pose_from_ref_to_new =
      relative_pose_from_ref_to_new;
  ...
  LOG_INFO(1) << "************frame registration: " << frame_ref.frame_index
              << " vs " << frame_new.frame_index << "************" << endl;
  ...
  frame_new_matcher.search_8(frame_ref.descriptor, init_matches,
                             g_para.hamming_distance_threshold);
  ...
  if (0) {
    ...
    std::vector<DMatch> no_matches;
    no_matches.clear();
    cv::drawMatches(frame_ref.rgb, frame_ref.keypoints, frame_new.rgb,
                    frame_new.keypoints, init_matches, img_matches);
    char frameMatchName[256];
    memset(frameMatchName, '\0', 256);
    //	sprintf(frameMatchName, "match_%04d_VS_%04d.jpg", frame_ref.frame_index,
    // frame_new.frame_index);
  ...
  reprojection_error = ransac3D3D(
      frame_ref, frame_new, init_matches, matches_before_filtering,
      g_para.reprojection_error_3d_threshold, g_para.ransac_maximum_iterations,
      fCorr, relative_pose_from_ref_to_new, camera_para);
  ...
  /********************** fine search **********************/
  {
    ...
    for (int i = 0; i < frame_ref.local_points.size(); i++) {
      ...
      homo_points << frame_ref.local_points[i](0), frame_ref.local_points[i](1),
          frame_ref.local_points[i](2), 1;
      ...
      predict_ref.pt.x =
          predict_points(0) * camera_para.c_fx + camera_para.c_cx;
      predict_ref.pt.y =
          predict_points(1) * camera_para.c_fy + camera_para.c_cy;
    ...
    init_matches.clear();
    frame_new_matcher.search_8_with_range(
        frame_ref.descriptor, init_matches, frame_new.keypoints,
        predict_ref_points, 30, g_para.hamming_distance_threshold * 1.5);
    // RefineByRotation(frame_ref, frame_new, init_matches);
    ...
    outlierFiltering(frame_ref, frame_new, init_matches);
    reprojection_error =
        ransac3D3D(frame_ref, frame_new, init_matches, complete_matches,
                   g_para.reprojection_error_3d_threshold,
                   g_para.ransac_maximum_iterations, fCorr,
                   relative_pose_from_ref_to_new, camera_para);
  ...
  scale_change_ratio =
      fmax(scale_decrease, scale_increase) / (init_matches.size() + 1);
  ...
  if (0) {
    LOG_INFO(1) << "average reprojection error : "
                << reprojection_error_3Dto3D(fCorr,
                                             (relative_pose_from_ref_to_new))
                << "    "
                << "average disparity: " << average_disparity << "    "
                << "scale change ratio: " << scale_change_ratio << endl;
    LOG_INFO(1) << "sparse match: " << sparse_success << " "
                << init_matches.size() << " " << reprojection_error << "    "
                << endl;
    LOG_INFO(1)
        << "run time: "
           "featureMatching/rotationFilter/filter/ransac/refine/finishing: "
        << time_feature_matching << "/" << time_rotation_filter << "/"
        << time_filter << "/" << time_ransac << "/" << time_refine << "/"
        << time_finishing << endl;
  }
...
void ComputeJacobianInfo(FrameCorrespondence &fC, Eigen::MatrixXd &Pre_JiTr,
                         Eigen::MatrixXd &Pre_JjTr, Eigen::MatrixXd &Pre_JiTJi,
                         Eigen::MatrixXd &Pre_JiTJj, Eigen::MatrixXd &Pre_JjTJi,
  ...
  Pre_JiTJi.block<3, 3>(0, 3) =
      -getSkewSymmetricMatrix(R_ref_sum_p_ref + fC.sum_weight * t_ref);
  Pre_JiTJi.block<3, 3>(3, 0) = -Pre_JiTJi.block<3, 3>(0, 3);
  Pre_JiTJi(3, 3) = riWri(2, 2) + riWri(1, 1);
  Pre_JiTJi(3, 4) = -riWri(1, 0);
  Pre_JiTJi(3, 5) = -riWri(2, 0);
  Pre_JiTJi(4, 3) = -riWri(0, 1);
  Pre_JiTJi(4, 4) = riWri(0, 0) + riWri(2, 2);
  Pre_JiTJi(4, 5) = -riWri(2, 1);
  Pre_JiTJi(5, 3) = -riWri(0, 2);
  Pre_JiTJi(5, 4) = -riWri(1, 2);
  Pre_JiTJi(5, 5) = riWri(0, 0) + riWri(1, 1);
  Pre_JiTJi.block<3, 3>(3, 3) +=
      -skewMatrixProduct(t_ref, R_ref_sum_p_ref) -
      skewMatrixProduct(R_ref_sum_p_ref, t_ref) -
      fC.sum_weight * 1 * skewMatrixProduct(t_ref, t_ref);

  Pre_JjTJj.block<3, 3>(0, 0) = Eye3x3 * fC.sum_weight;
  Pre_JjTJj.block<3, 3>(0, 3) =
      -getSkewSymmetricMatrix(R_new_sum_p_new + fC.sum_weight * t_new);
  Pre_JjTJj.block<3, 3>(3, 0) = -Pre_JjTJj.block<3, 3>(0, 3);
  Pre_JjTJj(3, 3) = rjWrj(2, 2) + rjWrj(1, 1);
  Pre_JjTJj(3, 4) = -rjWrj(1, 0);
  Pre_JjTJj(3, 5) = -rjWrj(2, 0);
  Pre_JjTJj(4, 3) = -rjWrj(0, 1);
  Pre_JjTJj(4, 4) = rjWrj(0, 0) + rjWrj(2, 2);
  Pre_JjTJj(4, 5) = -rjWrj(2, 1);
  Pre_JjTJj(5, 3) = -rjWrj(0, 2);
  Pre_JjTJj(5, 4) = -rjWrj(1, 2);
  Pre_JjTJj(5, 5) = rjWrj(0, 0) + rjWrj(1, 1);
  Pre_JjTJj.block<3, 3>(3, 3) +=
      -skewMatrixProduct(t_new, R_new_sum_p_new) -
      skewMatrixProduct(R_new_sum_p_new, t_new) -
      fC.sum_weight * 1 * skewMatrixProduct(t_new, t_new);

  Pre_JiTJj.block<3, 3>(0, 0) = Eye3x3 * fC.sum_weight;
  Pre_JiTJj.block<3, 3>(0, 3) =
      -getSkewSymmetricMatrix(R_new_sum_p_new + fC.sum_weight * t_new);
  Pre_JiTJj.block<3, 3>(3, 0) =
      -getSkewSymmetricMatrix(R_ref_sum_p_ref + fC.sum_weight * t_ref)
           .transpose();
  Pre_JiTJj(3, 3) = riWrj(2, 2) + riWrj(1, 1);
  Pre_JiTJj(3, 4) = -riWrj(1, 0);
  Pre_JiTJj(3, 5) = -riWrj(2, 0);
  Pre_JiTJj(4, 3) = -riWrj(0, 1);
  Pre_JiTJj(4, 4) = riWrj(0, 0) + riWrj(2, 2);
  Pre_JiTJj(4, 5) = -riWrj(2, 1);
  Pre_JiTJj(5, 3) = -riWrj(0, 2);
  Pre_JiTJj(5, 4) = -riWrj(1, 2);
  Pre_JiTJj(5, 5) = riWrj(0, 0) + riWrj(1, 1);
  Pre_JiTJj.block<3, 3>(3, 3) +=
      -skewMatrixProduct(t_ref, R_new_sum_p_new) -
      skewMatrixProduct(R_ref_sum_p_ref, t_new) -
      fC.sum_weight * 1 * skewMatrixProduct(t_ref, t_new);
  Pre_JiTJj = -Pre_JiTJj;
...
void addBlockToTriplets(std::vector<Eigen::Triplet<NumType>> &coeff,
                        Eigen::MatrixXd b, int start_x, int start_y) {
  int rows = b.rows();
  int cols = b.cols();
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < cols; j++) {
      coeff.push_back(
          Eigen::Triplet<NumType>(start_x + i, start_y + j, b(i, j)));
    }
  }
}
...
float initGraphHuberNorm(std::vector<FrameCorrespondence> &fCList,
                         std::vector<Frame> &F) {
  int origin = 0;
...
float optimizeKeyFrameMapRobust(
    vector<FrameCorrespondence> &fCList, vector<Frame> &F,
  ...
  for (int i = 0; i < fCList.size(); i++) {
    ...
    latest_keyframe_index = frame_ref.frame_index > latest_keyframe_index
                                ? frame_ref.frame_index
                                : latest_keyframe_index;
    latest_keyframe_index = frame_new.frame_index > latest_keyframe_index
                                ? frame_new.frame_index
                                : latest_keyframe_index;
  ...
  if (g_para.debug_mode) {
    double init_error =
        reprojection_error_3Dto3D(fCList, keyframe_candidate_fcorrs);
    LOG_INFO(1) << "init error		: " << init_error << endl;
  }

  // TODO will be replaced by conjugate gradient descent.
  ...
  Eigen::SimplicialLDLT<Eigen::SparseMatrix<SPARSE_MATRIX_NUM_TYPE>>
      SimplicialLDLTSolver;
  std::vector<Eigen::Triplet<SPARSE_MATRIX_NUM_TYPE>> coeff;
  coeff.reserve(6 * 6 * 4 * fCList.size());
  Eigen::MatrixXd JiTJi_pre(6, 6), JiTJj_pre(6, 6), JjTJi_pre(6, 6),
      JjTJj_pre(6, 6), JiTe_pre(6, 1), JjTe_pre(6, 1);
  ...
  for (int i = 0; i < keyframe_candidate_fcorrs.size(); i++) {
    ...
    if (frame_ref.frame_index == latest_keyframe_index ||
        frame_new.frame_index == latest_keyframe_index) {
      continue;
    }
  ...
  for (int iter = 0; iter < 3; iter++) {
    ...
    for (int i = 0; i < keyframe_candidate_fcorrs.size(); i++) {
      ...
      ComputeJacobianInfo(fCList[keyframe_candidate_fcorrs[i]], JiTe_pre,
                          JjTe_pre, JiTJi_pre, JiTJj_pre, JjTJi_pre, JjTJj_pre);
      ...
      if (frame_ref_pos == 0) {
        addBlockToTriplets(coeff, JjTJj_pre, (frame_new_pos - 1) * 6,
                           (frame_new_pos - 1) * 6);
        JTe.block<6, 1>((frame_new_pos - 1) * 6, 0) += JjTe_pre;
      } else {
        addBlockToTriplets(coeff, JiTJi_pre, (frame_ref_pos - 1) * 6,
                           (frame_ref_pos - 1) * 6);
        addBlockToTriplets(coeff, JiTJj_pre, (frame_ref_pos - 1) * 6,
                           (frame_new_pos - 1) * 6);
        addBlockToTriplets(coeff, JjTJi_pre, (frame_new_pos - 1) * 6,
                           (frame_ref_pos - 1) * 6);
        addBlockToTriplets(coeff, JjTJj_pre, (frame_new_pos - 1) * 6,
                           (frame_new_pos - 1) * 6);
    ...
    for (int i = 1; i < keyframes.size(); i++) {
      ...
      if (delta_i.array().isNaN().maxCoeff() > 0 ||
          isnan(sin(delta_i.array().abs().maxCoeff()))) {
        cout << "nan detected in pose update! " << endl;
        continue;
      }
      // FIXME: sometimes exp failure here because of extreme delta_i value;
      F[keyframes[i]].pose_sophus[0] =
          Sophus::SE3d::exp(delta_i).inverse() * F[keyframes[i]].pose_sophus[0];
    }
    ...
    if (g_para.debug_mode) {
      ...
      for (int i = 0; i < keyframe_candidate_fcorrs.size(); i++) {
        average_reprojection_error +=
            reprojection_error_3Dto3D(fCList[keyframe_candidate_fcorrs[i]]) *
            fCList[keyframe_candidate_fcorrs[i]].sum_weight;
        count += fCList[keyframe_candidate_fcorrs[i]].sum_weight;
      }
      refined_error = average_reprojection_error / count;

      end = clock();
      time_calculate_reprojection_error =
  ...
  for (int i = 0; i < kflist.size(); i++) {
    for (int j = 0; j < kflist[i].corresponding_frames.size(); j++) {
      ...
      F[kflist[i].corresponding_frames[j]].pose_sophus[0] =
          F[kflist[i].keyFrameIndex].pose_sophus[0] *
    ...
    for (int i = 0; i < keyframe_candidate_fcorrs.size(); i++) {
      ...
      if (frame_ref.frame_index == latest_keyframe_index ||
        ...
        if (fabs(frame_ref_pos - frame_new_pos) < min_dist &&
            fCList[keyframe_candidate_fcorrs[i]].matches.size() > 0) {
          cout << "try to remove: " << fabs(frame_ref_pos - frame_new_pos)
               << " " << min_dist << " " << frame_ref.frame_index << " "
               << frame_new.frame_index << endl;
...
float reprojection_error_3Dto3D(
    const FrameCorrespondence &fC,
    const Sophus::SE3d &relative_pose_from_ref_to_new) {
  Eigen::MatrixXd R_ref = relative_pose_from_ref_to_new.rotationMatrix();
  Eigen::Vector3d t_ref = relative_pose_from_ref_to_new.translation();
  ...
  if (fC.sum_weight > 0) {
    total_error = fC.sum_p_ref_ref(0, 0) + fC.sum_p_ref_ref(1, 1) +
                  fC.sum_p_ref_ref(2, 2) + fC.sum_p_new_new(0, 0) +
                  fC.sum_p_new_new(1, 1) + fC.sum_p_new_new(2, 2) +
                  fC.sum_weight * t_ref.transpose() * t_ref -
                  2 * (float)(t_ref.transpose() * fC.sum_p_new) +
                  2 * (float)(t_ref.transpose() * R_ref * fC.sum_p_ref) -
...
float reprojection_error_3Dto3D(const FrameCorrespondence &fC) {
  return reprojection_error_3Dto3D(
      fC, fC.frame_new.pose_sophus[0].inverse() * fC.frame_ref.pose_sophus[0]);
}
...
float reprojection_error_3Dto3D(std::vector<FrameCorrespondence> fCList) {
  ...
  for (int i = 0; i < fCList.size(); i++) {
    average_reprojection_error +=
        reprojection_error_3Dto3D(fCList[i]) * fCList[i].sum_weight;
...
float reprojection_error_3Dto3D(std::vector<FrameCorrespondence> fCList,
  ...
  for (int i = 0; i < candidates.size(); i++) {
    average_reprojection_error +=
        reprojection_error_3Dto3D(fCList[candidates[i]]) *
        fCList[candidates[i]].sum_weight;
    count += fCList[candidates[i]].sum_weight;
  }
  return average_reprojection_error / count;
}
float reprojection_error_3Dto3D_perFrame(
    std::vector<FrameCorrespondence> fCList, Eigen::VectorXd &errorPerFrame,
    Eigen::VectorXd &pointsPerFrame, Eigen::VectorXd &connectionsPerFrame) {
  ...
  for (int i = 0; i < fCList.size(); i++) {
    ...
    errorPerFrame[ref_frame_index] +=
        reprojection_error_3Dto3D(fCList[i]) * fCList[i].sum_weight;
    errorPerFrame[new_frame_index] +=
        reprojection_error_3Dto3D(fCList[i]) * fCList[i].sum_weight;
...
float reprojection_error_3Dto3D(Point3dList pt_ref, Point3dList pt_new,
  ...
  if (use_huber_norm) {
    for (int i = 0; i < pt_ref.size(); i++) {
      Eigen::Vector3d reprojection_error =
          applyPose(relative_pose_from_ref_to_new, pt_ref[i]) - pt_new[i];
...
float reprojection_error_3Dto3D(Frame frame_ref, Frame frame_new,
  ...
  return reprojection_error_3Dto3D(p_ref, p_new, relative_pose_from_ref_to_new,
                                   use_huber_norm, huber_norm_threshold);
}

Eigen::Matrix3d skewMatrixProduct(Eigen::Vector3d t1, Eigen::Vector3d t2) {
  Eigen::Matrix3d M;
  M(0, 0) = -t1(1) * t2(1) - t1(2) * t2(2);
  M(0, 1) = t1(1) * t2(0);
  M(0, 2) = t1(2) * t2(0);
  M(1, 0) = t1(0) * t2(1);
  M(1, 1) = -t1(2) * t2(2) - t1(0) * t2(0);
  M(1, 2) = t1(2) * t2(1);
  M(2, 0) = t1(0) * t2(2);
  M(2, 1) = t1(1) * t2(2);
  M(2, 2) = -t1(1) * t2(1) - t1(0) * t2(0);
  return M;
}
